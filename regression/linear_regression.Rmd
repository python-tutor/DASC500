...
which measures the number of standard deviations that $\beta_1$ is away from 0.  Thus a large *t-statistic* such as ours will produce a small *p-value* (a small p-value indicates that it is unlikely to observe such a substantial association between the predictor variable and the response due to chance).  Thus, we can conclude that a relationship between TV advertising budget and sales exists.
...
Here we are looking for outlying values (we can select the top *n* outliers to report with `id.n`.  The identified (labeled) points represent those plots where cases can be influential against a regression line. When cases have high Cook's distance scores and are to the upper or lower right of our leverage plot they have *leverage* meaning they are influential to the regression results. The regression results will be altered if we exclude those cases.  
